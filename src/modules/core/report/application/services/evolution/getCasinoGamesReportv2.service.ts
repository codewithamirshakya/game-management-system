...
import {GetCasinoGamesDto} from "../../dtos/request/evolution/getCasinoGames.dto";

export class GetCasinoGamesReportv2Service {
    constructor(
    ...
    @Transactional()
    public async get(dto: GetCasinoGamesDto,req: Request,ip: string) {
        try {
            const response = await this.repo.getReport(dto,'/api/gamehistory/v1/casino/games/stream');
            ...
            await this.eventDispatcher.dispatch(EventDefinition.ACTIVITY_COMPLETED_EVENT,
                new ActivityCompletedEvent(
                    ...
                    "[Casino games report fetched successfully.]",
                    ip,
