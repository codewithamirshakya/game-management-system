...
import {GetCasinoDailyReportDTO} from "../../dtos/request/evolution/getCasinoDailyReport.dto";

export class GetCasinoDailyReportService {
    constructor(
    ...
    @Transactional()
    public async get(dto: GetCasinoDailyReportDTO,req: Request,ip: string) {
        try {
            const response = await this.repo.getReport(dto,'/api/gamehistory/v1/casino/daily-report');
            ...
            await this.eventDispatcher.dispatch(EventDefinition.ACTIVITY_COMPLETED_EVENT,
                new ActivityCompletedEvent(
                    ...
                    "[Casino daily report fetched successfully.]",
                    ip,
