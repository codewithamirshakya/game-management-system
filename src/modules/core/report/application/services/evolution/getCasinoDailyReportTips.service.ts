...
export class GetCasinoDailyReportTipsService {
    constructor(
    ...
    public async get(dto: GetCasinoDailyReportTipsDto,req: Request,ip: string) {
        try {
            const response = await this.repo.getReport(dto,'/api/gamehistory/v1/casino/daily-report-tips');
            ...
            await this.eventDispatcher.dispatch(EventDefinition.ACTIVITY_COMPLETED_EVENT,
                new ActivityCompletedEvent(
                    ...
                    "[Casino daily report tips fetched successfully.]",
                    ip,
